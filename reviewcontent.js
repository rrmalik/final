...
firebase.auth().onAuthStateChanged(async function(user) {
  ...
  // Populate curated by: 
    ...
    <div class="pl-4 md:pl-32 w-1/8 rounded-lg text-left font-dark font-normal text-small "><strong>curated by:</strong>${curatedBy.join().toLowerCase()}</div>
  `
  ...
  // Hide the "Join Group" "button 
      ...
      // drop user information into user-group mapping table
        ...
        // REpopulate curated by: 
            ...
            document.querySelector('.curatedBy').innerHTML = `
            <div class="pl-12 md:pl-32 w-1/8 rounded-lg text-left font-dark font-normal text-small "><strong>curated by:</strong>${curatedBy.join().toLowerCase()}</div>
          `
      })
    }
    ...
    <div class="text-6xl text-left pl-4 md:pl-32 font-bold">${groupName}</div>
     `
...
async function renderContent(contentUrl, contentTitle, contentAuthor, contentTime, contentUserId, contentDisplayName, contentCommentary) {
  ...
  <div class="grid pl-4 sm:pl-32 gap-0 grid-cols-1 align-left">
